...
- A SUBAGENT WAITS IN THE FOREGROUND. A harness often does not wake a subagent
  when a background command ends: it resumes the subagent only when the parent
  sends a message. A subagent that ends its turn to wait then stays stopped
  until its parent notices. Use `qex submit --wait`, or
