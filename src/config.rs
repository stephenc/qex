...
"#;
    ...
    /// A pool of zero units admits no job at all. The refusal must name the
    /// field: a pool that silently holds nothing looks exactly like a pool
    /// that is always full, and a reader cannot tell the two apart.
    #[test]
    fn a_pool_with_a_count_of_zero_is_refused() {
        // The pre-condition: the same file with `count = 1` is accepted.
