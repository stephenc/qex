...
pub fn stream(
...

    {
        ...
        lead.push(Event::Stream {
            time: now,
            // The version that `qex info` reports, and not the number in
            // `Cargo.toml`. `main` holds `0.0.0-dev` for ever, so the manifest
            // number names no build; `version::VERSION` comes from `build.rs`
