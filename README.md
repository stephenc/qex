...
**A claim is a promise — but the job is told what it promised.** qex writes the
claim into the environment (`GOMAXPROCS`, `OMP_NUM_THREADS`, `GOMEMLIMIT`,
`NODE_OPTIONS`, `QEX_CPU` and more), so a runtime sizes its thread pool to the
claim instead of to the machine. It never replaces a value you set yourself. A
program that asks the operating system directly still sees the whole machine.
